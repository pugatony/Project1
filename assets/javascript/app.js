    ...
    query.once("value")
        .then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                $("#dress-information").append("<div class = 'col-sm-6'><div class = 'card mx-auto' style = 'width: 18rem;'><img src = '" + childSnapshot.val().image + "' class = 'card-img-top' alt = '...' ><div class = 'card-body'> <h5 class = 'card-title dress' id = 'dress'>" + childSnapshot.val().model_id + "</h5></div><ul class='list-group list-group-flush'><li class='list-group-item description' id='description'>" + childSnapshot.val().description + "</li><li class='list-group-item line' id='line'>" + childSnapshot.val().line + "</li><li class='list-group-item quantity' id='quantity'>" + childSnapshot.val().quantity + "</li></ul><div class= 'card-body'><a href='cart.html' class='btn btn-primary'>BUY</a></div></div></div>");
            });
        });
})